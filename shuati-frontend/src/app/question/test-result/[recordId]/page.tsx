...
import { getQuestionTestRecordVoUsingGet } from "@/api/questionTestController";
...
export default function QuestionTestResultPage({ params }: { params: { recordId: string } }) {
  const { recordId } = params;
  const [loading, setLoading] = useState(true);
  const [result, setResult] = useState<QuestionTestResult>();
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const res = await getQuestionTestRecordVoUsingGet({
          recordId,
        });
        setResult(res.data);
      } catch (e: any) {
  ...
  return (
    <div
      style={{
        minHeight: "100vh",
        background:
          "radial-gradient(circle at top, rgba(22, 119, 255, 0.08), transparent 28%), #f5f7fb",
          ...
          {result.itemResultList.map((item, index) => (
            ...
            >
              ...
              {item.analysis ? (
                ...
                >
                  <Typography.Text style={{ display: "block", fontSize: 16, color: "#64748b", marginBottom: 8 }}>
                    解析
                  </Typography.Text>
